<!-- sidebar start-->

<div class="col-2 p-0 position-fixed start-0 sidebar">
          <div class="bottom-0 position-relative bg-white"  style=" height: 648px;">
            <div class="d-flex flex-column flex-shrink-0 bars-tabs">
                <ul class="nav flex-column mb-auto sidebar-menu">
                  <li class="nav-item py-1">
                    <a href="dashborad.php" class="nav-link text-dark py-2 pe-5" aria-current="page">
                        <i class="bi bi-house-door me-2 fs-5"></i>
                     Dashborad
                    </a>
                  </li>
                  <li>
                    <a href="#" class="nav-link text-dark py-2 pe-5">
                        <i class="bi bi-houses me-2 fs-5" aria-label="Orders"></i>
                      Profile
                    </a>
                  </li>
                  <li>
                    <a href="#" class="nav-link text-dark py-2 pe-5">
                        <i class="bi bi-hospital me-2 fs-5"></i>
                      Change Password
                    </a>
                  </li>
                  <li class="nav-item py-1"  data-bs-toggle="collapse" href="#collapseExample" role="button" aria-expanded="false" aria-controls="collapseExample">
                    <a href="View_property.php" class="nav-link text-dark py-2 pe-5" aria-current="page">
                        <i class="bi bi-house-door me-2 fs-5"></i>
                      My Property
                    </a>
                  </li>
                  <div class="collapse border-0" id="collapseExample">
                  <div class="card card-body">
                        <ul class="sub-menu">
                          <li>
                            <a href="Add_property.php">Add Property</a>
                          </li>
                          <li>
                            <a href="View_property.php">View Property 1</a>
                          </li>
                        </ul>
                  </div>
                  </div>
                  <li>
                    <a href="#" class="nav-link text-dark py-2 pe-5">
                        <i class="bi bi-person-circle me-2 fs-5"></i>
                      Recieved Inquirey 
                    </a>
                  </li>
                  <li>
                    <a href="#" class="nav-link text-dark py-2 pe-5">
                        <i class="bi bi-person-circle me-2 fs-5"></i>
                        Answerd Query
                    </a>
                  </li>
                  <li>
                    <a href="#" class="nav-link text-dark py-2 pe-5">
                        <i class="bi bi-person-circle me-2 fs-5"></i>
                        Logout
                    </a>
                  </li>
                  
                </ul>
            </div>
        </div>
</div>
<!-- sidebar start-->